...
public class Helpers
{
    ...
    public static bool IsQuizInLegitGuidList(string WebAppId, IWebHostEnvironment env, HttpRequest request)
    {
        string[] guids = GetQuizGuids();
        bool isOk = false;
        foreach (var g in guids)
        {
            if (string.Equals(g!.Trim(), WebAppId.Trim(), StringComparison.OrdinalIgnoreCase))
            {
                isOk = true;
                break;
            }
    ...
    public static string[] GetQuizGuids() {
        string[] p = { Directory.GetCurrentDirectory(), "wwwroot", "guids.txt" };
        var guidsFilePath = Path.Combine(p);
        string[] guids = File.ReadAllLines(guidsFilePath);
        return guids;
    }
